...
class DiagnosticChatbot {
    ...
    async analyzeImage() {
        ...
        try {
            ...
            // ✅ Parse JSON avant de vérifier le statut pour gérer les erreurs métier
            const result = await response.json();

            // Hide typing
            this.hideTyping();

            // ✅ NOUVEAU: Vérifier si l'image n'est pas une tomate
            if (!response.ok) {
                // Code 400 avec erreur métier (image non-tomate)
                if (response.status === 400 && result.error === 'image_not_tomato') {
                    this.sendBotMessage(`❌ **Image introuvable**\n\n${result.errorMessageFr || 'Cette image ne semble pas être une plante de tomate. Veuillez utiliser une photo de feuilles de tomate.'}\n\n📸 Essayez avec une photo claire de feuilles de tomate !`);
                    this.removeImage();
                    return;
                }
                // Autre erreur
            ...
            // ✅ Vérifier aussi au niveau du résultat
            if (result.success === false && result.error === 'image_not_tomato') {
                this.sendBotMessage(`❌ **Image introuvable**\n\n${result.errorMessageFr || 'Cette image ne semble pas être une plante de tomate. Veuillez utiliser une photo de feuilles de tomate.'}\n\n📸 Essayez avec une photo claire de feuilles de tomate !`);
                this.removeImage();
                return;
            }
